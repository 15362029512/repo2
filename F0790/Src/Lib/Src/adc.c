...
返回值：  无
***********************************************************************************/
void ADC_init(ADC_RefVolt_TypeDef adc_ref,
							InterruptSW_TypeDef adc_int,
							AmpSW_TypeDef amp_sw,
							AMP_Gain_TypeDef gain)							
{
...
返回值：  ADC值
***********************************************************************************/
unsigned int GetChannelAdcValue(ADC_Channel_TypeDef channel)
{
	unsigned int ADC_Value;
	ADC_ConfigChannel(channel);//配置ADC通道
	ADC_StartConversion();//启动ADC转换
	while(!(ADCON & ADIF));//等待转换完成
	ADCON |= ADIF;//清除中断(转换完成)标志
	ADC_Value = ADCDH*256 + ADCDL;
	ADC_Value >>= 4;//获取AD值	
	return ADC_Value;
}
